 ...
 *  never `task_type` — but a refusal the drafter cannot read is worse than a
 *  verbose map, and the server's own message is the fallback. */
...
function toRequest(values: FormState): NodeMetadataRequest {
  return {
    ...
    applicability: list(values.applicability),
    legal_provision: list(values.legal_provision),
    issuance_date: text(values.issuance_date),
    effective_date: text(values.effective_date),
    policy_requirement: list(values.policy_requirement),
...
/**
 * Edit mode for the Metadata disclosure: seven controlled inputs, Save and
 ...
 * actions; a seven-field form inline would make it the largest file in the
 * feature.
 */
  ...
  return (
    <div className="mt-2 space-y-3">
      {CONCEPT_FIELD_ORDER.map(([field, label]) => {
        ...
        // rather than leaving the drafter to guess which of seven it means.
        const inputClass = cn(
          fieldClass,
