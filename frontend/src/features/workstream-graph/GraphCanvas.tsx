...
export function GraphCanvas({
  ...
  const drawNode = useCallback(
    (node: FGNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      ctx.strokeStyle = selected ? "#1e293b" : style.stroke;
      ctx.stroke();
      ctx.restore();
  ...
  return (
    ...
    >
      <div className="absolute right-3 top-3 z-10 flex flex-col overflow-hidden rounded-lg border border-border/60 bg-card shadow-sm">
        <button
          type="button"
          aria-label="Zoom in"
          onClick={zoomIn}
