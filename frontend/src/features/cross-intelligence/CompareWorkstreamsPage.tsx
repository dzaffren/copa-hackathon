...
export function CompareWorkstreamsPage() {
  ...
  // Default to the flagship pair (OpRes vs Open Finance) when present.
  const fallbackA = pick(workstreams, ["opres-v2", "rmit-v2-2025"]);
  const fallbackB = pick(workstreams, ["open-finance-ed", "rmit-v2-2025"]);
  ...
  return (
    <label className="flex items-center gap-1.5 text-sm">
      ...
      <select
        ...
        className="rounded-lg border border-border/60 bg-background/60 px-2 py-1.5 text-sm outline-none focus:border-primary/50"
      >
        {workstreams
