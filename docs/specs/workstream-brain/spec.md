...
- ~~**Live network calls to Azure AI Foundry during the demo.**~~ **Retired by the Arm G engine promotion.** `Analyze linkages` now makes **live** model calls through the Arm G pipeline (Haiku extraction → Sonnet batched same-topic finder → Opus whole-doc coverage), not a replayed trace. See [spec-engine-arm-g-promotion.md](spec-engine-arm-g-promotion.md).

## Story Index

| Ticket | Story                                              | Spec                                                                     | Type        | Status      | Dependencies                  |
| ------ | -------------------------------------------------- | ------------------------------------------------------------------------ | ----------- | ----------- | ----------------------------- |
| TBD    | Linkage taxonomy widening in the engine            | [spec-engine-taxonomy.md](spec-engine-taxonomy.md)                       | Technical   | Done        | —                             |
| TBD    | Anchor segmentation (multi-strategy `AnchorIndex`) | [spec-engine-anchor-segmentation.md](spec-engine-anchor-segmentation.md) | Technical   | Done        | Taxonomy                      |
| TBD    | Arm G engine promotion (coverage-aware pipeline)   | [spec-engine-arm-g-promotion.md](spec-engine-arm-g-promotion.md)         | Technical   | Done        | Taxonomy; Anchor segmentation |
...
| TBD    | Task screen with pairwise comparison               | [spec-task-screen.md](spec-task-screen.md)                               | User-facing | Not Started | Graph; Arm G engine           |
| TBD    | Review linkages screen                             | [spec-review-linkages.md](spec-review-linkages.md)                       | User-facing | Not Started | Graph; Arm G engine           |
...
- **Pairs go through the Arm G coverage-aware pipeline.** `Analyze linkages` runs a deterministic-orchestrator pipeline with role-specialised, right-sized model stages: axis extraction (small model) → cosine retrieval with automatic BM25 fallback → batched same-topic finder (mid model) → suppression build → whole-document coverage finder (large model) → merge + deterministic citation validation. Same-topic labels come from per-pair judgment; the two coverage labels (`silent-on`/`goes-beyond`) come from a whole-document pass so absence is verified against the entire other document. No critic stage, no free-form multi-agent conversation, no glossary. See [spec-engine-arm-g-promotion.md](spec-engine-arm-g-promotion.md).
...
- **Existing knowledge-graph engine.** The pairwise linkage discovery loop, the clause index, and the multi-strategy `AnchorIndex` are built and validated. The taxonomy-widening story extends the label vocabulary; the anchor-segmentation story widens what can be cited; the **Arm G engine promotion** replaces raw whole-document pairwise prompting with the coverage-aware pipeline (axis extraction → cosine/BM25 retrieval → batched same-topic finder → whole-document coverage finder → validation) and wires it onto the live `Analyze linkages` route. Every user-facing story consumes them. See [spec-engine-arm-g-promotion.md](spec-engine-arm-g-promotion.md).
- **Live Arm G analysis.** `Analyze linkages` runs the Arm G pipeline live per edge and persists findings per-link on disk (keyed by workstream + edge). The earlier "replay the retired experiment trace" backstop (`data/artifacts/connection-trace-opres-v1-2025-draft__open-finance-v1-2025-ed.json`) is superseded; the trace remains only as historical reference.
...
- ~~**Cross-jurisdiction glossary (`data/glossary.json`).**~~ **Dropped by the Arm G engine promotion.** The glossary-expansion retrieval stage is not part of the shipping pipeline; cross-vocabulary matching is handled by cosine similarity (with BM25 fallback). The glossary file, if present, is unused by the live engine.
