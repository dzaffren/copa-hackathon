...
| **Retrieval candidate count**     | `len(candidates)` — the number of `(source_anchor, target_anchor)` pairs the retrieval stage hands to the per-pair finder+critic loop. Recorded as `n/a` for Arm B (see note below).                                                        | The retrieval "budget" — how many pairs the LLM spends calls on. Lets us compare how tight each arm's filter is.         |

**Why `candidates = n/a` for Arm B.** Arm B is whole-doc pairwise — both documents' full anchor lists are handed to the finder in a single prompt, and the LLM itself proposes pairs from that context. There is no separate Stage-B retrieval step that produces a filtered candidate list, so there is nothing to count. Arm B's cost scales with total anchor tokens (~216 anchors in one prompt) rather than with a candidate count. For Arms C/D/E/F, `candidates` is what survived per-anchor top-K plus a global cap (currently 60 pairs) and was handed downstream — this is why C/D/E/F all land near 60 candidates per pair, while C on the smaller HKMA target document lands below the cap.
